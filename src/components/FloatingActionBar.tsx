...
export function FloatingActionBar() {
  ...
  const createSimpleFlow = () => {
    ...
    const imageNodeId = addNode("imageInput", imagePosition);
    const promptNodeId = addNode("prompt", promptPosition);
    const generateNodeId = addNode("nanoBanana", generatePosition);
    ...
    return { imageNodeId, promptNodeId, generateNodeId };
  };

  const handleCreateSimpleFlow = () => {
    createSimpleFlow();
  };

  const handleCreateWhiteBgTemplate = () => {
    const { imageNodeId, promptNodeId } = createSimpleFlow();
      ...
      <div className="flex items-center gap-0.5 bg-neutral-800/95 backdrop-blur-sm rounded-lg shadow-lg border border-neutral-700/80 px-1.5 py-1">
        ...
        <div className="relative">
          ...
          >
            Template
            <svg
              className={`w-3 h-3 transition-transform ${templateMenuOpen ? "rotate-180" : ""}`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={2}
